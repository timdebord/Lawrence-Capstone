...
circle_radius <- 535  # Your specified distance threshold
...
neighbors <- dnearneigh(centroids_sp, 0, 535)
...
neighbors[[5]] 
model_ready$State[c(14 ,25 ,29 ,42 ,48)] # adjust based on neighbors[[i]]
...
leaflet(state_damage_2) %>%
  addTiles() %>%  # Add base tiles
  addPolygons(fillColor = ~colorNumeric("YlOrRd", actual_NRI)(actual_NRI),
              color = "black", weight = 1, fillOpacity = 0.7,
              popup = ~paste(State, "<br> NRI: ", actual_NRI)) %>%
  addLegend("bottomright", 
            pal = colorNumeric("YlOrRd", state_damage_2$actual_NRI, n = 8),
            values = state_damage_2$actual_NRI,
            title = "Actual NRI",
            opacity = 1)
```
...
vars_of_interest <- model_ready[, c("State", "homePrems", "home_damage", "disaster_damage", "AvgHomePrice", "Percent_white", "PopDensity", "actual_NRI")]
...
kable(table, format = "html", table.attr = "style='width:100%;'") %>%
  ...
  column_spec(8, border_right = TRUE)  # Adds vertical lines to the eighth column
